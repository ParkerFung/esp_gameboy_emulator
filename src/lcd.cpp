...
//-------------------------------------------------------------------------
static byte getPixelFromFramebuffer(int x, int y, byte *b) {
    int offset = x + y * 160;
    int byte_index = offset >> 2;
    int bit_pos = (offset & 3) << 1;
    return (b[byte_index] >> bit_pos) & 0x3;
}
//---------------------------------------------------------------------------
...
void drawColorIndexToFrameBuffer(int x, int y, byte idx, byte *b) {
    ...
    int byte_index = offset >> 2;
    int bit_pos = (offset & 3) << 1;
    b[byte_index] &= ~(0x3 << bit_pos);  // Clear the 2 bits for this pixel
    b[byte_index] |= (idx << bit_pos);   // Set new 2-bit pixel value
}
//------------------------------------------------------------------------------
	...
	for(i = 0; i < nsprites; i++)
	{
		...
		for(x = 0; x < 8; x++)
		{
			...
			if(s[i].flags & PRIO)
			{
				...
				if(bg_pixel != bgpalette[0])  // Only draw sprite pixel if BG pixel is palette 0
					continue;
			}
//-------------------------------------------------------------------------------------------------
